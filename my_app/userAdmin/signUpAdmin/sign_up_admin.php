...
if (!empty($_POST)) {
  ...
  if (!$error_message) {
    ...
    if (!empty($data)) {
      ...
      $count = $stmt->rowCount();
      $count_message = "新規ユーザーを{$count}人追加しました";

      $host = $_SERVER['HTTP_HOST'];
      $dir = dirname($_SERVER['PHP_SELF'], 2);
      header("Location: //$host$dir/userAdmin.php?register_count={$register_count}");
      exit;
    }
  }
} else {
...
<head>
  ...
  <link rel="stylesheet" href="../../loginFunction/signUp/signUp.css">
</head>
